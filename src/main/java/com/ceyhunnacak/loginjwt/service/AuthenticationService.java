...
public class AuthenticationService {
    ...
    public User signup(RegisterUserDto input) {
        ...
        user.setVerifitacionCodeExpiresAt(LocalDateTime.now().plusMinutes(15));
    ...
    public void verifyUser(VerifyUserDto input) {
        ...
        if (optionalUser.isPresent()) {
            ...
            if (user.getVerifitacionCodeExpiresAt().isBefore(LocalDateTime.now())) {
                throw new RuntimeException("Verification code expired");
            }
            if (user.getVerificationCode().equals(input.getVerificationCode())) {
                user.setEnabled(true);
                user.setVerificationCode(null);
                user.setVerifitacionCodeExpiresAt(null);
    ...
    public void resendVerificationCode(String email) {
        ...
        if (optionalUser.isPresent()) {
            ...
            user.setVerificationCode(generateVerificationCode());
            user.setVerifitacionCodeExpiresAt(LocalDateTime.now().plusHours(1));
            sendVerificationEmail(user);
            userRepository.save(user);
        } else {
            throw new UsernameNotFoundException("Username not found");
